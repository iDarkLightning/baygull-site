...
import { article, usersToArticles, articlesToTopics, user, topic, session, articleMedia, publishMeta, draftMeta, archiveMeta, publishDefaultContent, draftDefaultContent, graphicContent, } from "./schema";
...
export const publishDefaultContentRelations = relations(publishDefaultContent, ({ one }) => ({
    article: one(article, {
        fields: [publishDefaultContent.articleId],
        references: [article.id],
    }),
}));
export const draftDefaultContentRelations = relations(draftDefaultContent, ({ one }) => ({
    article: one(article, {
...
export const articlesToTopicsRelations = relations(articlesToTopics, ({ one }) => ({
    article: one(article, {
